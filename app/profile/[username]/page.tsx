    ...
    return (
      <MainLayout>
        <div className="flex flex-col items-center justify-center h-[50vh] p-8 text-center">
          ...
          <p className="text-secondary-foreground opacity-60">Try searching for another.</p>
        </div>
      </MainLayout>
    );
  }
        ...
        {activeTab === "posts" && (
          <>
            {posts.isLoading ? (
              ...
              <>
                {allPosts.length > 0 ? (
                  allPosts.filter(Boolean).map((post: Post) => <PostCard key={post.id} post={post} />)
                ) : (
            ...
            {articles.isLoading ? (
              ...
              <>
                {allArticles.length > 0 ? (
                  allArticles.filter(Boolean).map((article: Article) => <ArticleCard key={article.id} article={article} />)
                ) : (
        ...
        {activeTab === "likes" && (
          <>
            {likes.isLoading ? (
              ...
              <>
                {allLikes.length > 0 ? (
                  allLikes.filter(Boolean).map((item, index) => (
    ...
    <div className="flex flex-col p-12 items-center text-center">
      <div className="h-16 w-16 rounded-full bg-secondary-ui flex items-center justify-center mb-4">
        {type === "likes" ? (
          ...
          <div className="h-8 w-8 text-primary-ui opacity-20 font-bold text-2xl uppercase">{type[0] || "?"}</div>
        )}
      </div>
