...
const ThinkingBlock = ({ duration, reasoning, loading }: { duration?: number, reasoning: string, loading?: boolean }) => {
  ...
  const formatDuration = (val?: number) => {
    ...
    let totalSeconds = val > 1_000_000 ? Math.floor(val / 1_000_000_000) : Math.floor(val / 1000);
    ...
    <div className="mb-4 flex flex-col items-start w-full">
      ...
      >
        <div className="w-[20px] shrink-0 flex justify-center">
          <Brain className={cn("h-[18px] w-[18px]", loading && !duration ? "animate-pulse text-primary-ui" : "")} strokeWidth={2.5} />
        </div>
        <span className="text-[14px]">
          {loading && !duration ? "Thinking..." : `Thought for ${durationText}`}
        </span>
        <ChevronDown className={cn("h-[15px] w-[15px] opacity-40 transition-transform", expanded ? "rotate-180" : "")} />
      </button>
...
export default function FernPage() {
  ...
  const handleSend = async (e?: React.FormEvent) => {
    ...
    const assistantMsg = { role: "assistant", content: "", reasoning: "", loading: true, createdAt: new Date().toISOString() };
        ...
        <header className="flex h-16 shrink-0 items-center justify-between border-b px-4 bg-background z-20">
          <div className="flex items-center gap-3">
            <div className="flex h-11 w-11 items-center justify-center rounded-xl overflow-hidden text-background shadow-lg">
              <Image src="/images/fern.webp" alt="Fern AI" width={44} height={44} className="h-full w-full object-cover" />
            </div>
            <div>
              <div className="flex items-center gap-2 relative">
                ...
                >
                  Fern AI <ChevronDown className="h-5 w-5 text-foreground/50 transition-transform" style={{ transform: showModelDropdown ? 'rotate(180deg)' : 'none' }} />
                </button>
                <AnimatePresence>
                  {showModelDropdown && (
                    <motion.div
                    ...
                    >
                      {AI_MODELS.map(m => (
                        <button
                          ...
                          className={cn("w-full text-left px-4 py-2.5 text-sm hover:bg-secondary-ui flex items-center justify-between transition-colors", selectedModel === m.id && "text-primary-ui bg-primary-ui/5")}
                        >
                          <div className="flex flex-col">
                            <span className="font-bold">{m.name}</span>
                            <span className="text-[10px] opacity-60 uppercase tracking-widest">{m.provider}</span>
                          </div>
                          {selectedModel === m.id && <Check className="h-4 w-4" />}
                        </button>
                      ))}
                    </motion.div>
                  )}
              ...
              <div className="flex items-center gap-1.5 mt-0.5">
                ...
                <span className="text-[10px] font-bold tracking-widest text-emerald-500 uppercase">{AI_MODELS.find(m => m.id === selectedModel)?.name} Online</span>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-1.5 p-1 bg-secondary-ui/50 rounded-2xl border">
            <button
              ...
              className={cn(
                ...
                showHistory ? "bg-background text-primary-ui shadow-md" : "text-foreground/60"
              )}
              title="Chat History"
            >
            ...
            <div className="flex flex-col items-center justify-center h-full text-center w-full max-w-2xl mx-auto space-y-6">
              ...
              >
                <div className="h-28 w-28 rounded-3xl overflow-hidden flex items-center justify-center shadow-2xl border border-border-ui/50">
                  <Image src="/images/fern.webp" alt="Fern AI" width={112} height={112} className="h-full w-full object-cover" />
                </div>
              ...
              <div>
                <h2 className="text-[32px] font-semibold text-foreground/90">Hello! I&apos;m Fern</h2>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 w-full max-w-2xl mt-8">
                {[
                  ...
                  >
                    <span className="text-xl group-hover:scale-110 transition-transform">{hint.icon}</span>
                    <span className="text-[14px] font-medium opacity-80 group-hover:text-primary-ui transition-colors leading-snug">{hint.text}</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            <>
              {messages.map((msg, i) => (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  className={cn("flex flex-col w-full max-w-4xl mx-auto mb-6", msg.role === "user" ? "items-end" : "items-start")}
                >
                  {msg.role === "user" ? (
                    ...
                    <div className="flex flex-col items-start w-full">
                      {(msg.reasoning || msg.loading) && (
                        <ThinkingBlock duration={msg.duration} reasoning={msg.reasoning || ""} loading={msg.loading} />
                      )}
            ...
            <div className="flex items-center justify-between p-3 pt-1">
              <div className="flex items-center gap-2">
                <button
                  ...
                  className={cn("flex items-center gap-1.5 border rounded-full px-3 py-1.5 text-[13px] font-semibold transition-colors",
                    enableReasoning ? "text-primary-ui border-primary-ui/30 bg-primary-ui/10 hover:bg-primary-ui/20" : "text-foreground/70 border-border-ui bg-background/50 hover:bg-secondary-ui")}
                >
                ...
                <button
                  ...
                  className={cn("flex items-center gap-1.5 border rounded-full px-3 py-1.5 text-[13px] font-semibold transition-colors",
                    enableWebSearch ? "text-primary-ui border-primary-ui/30 bg-primary-ui/10 hover:bg-primary-ui/20" : "text-foreground/70 border-border-ui bg-background/50 hover:bg-secondary-ui")}
                >
              ...
              <div className="flex items-center gap-2">
                <button type="button" className="p-2 text-foreground/50 hover:text-foreground transition-colors">
            ...
            <>
              ...
              >
                <div className="h-16 flex items-center justify-between px-6 border-b">
                  ...
                  <button onClick={() => setShowHistory(false)} className="p-2 hover:bg-secondary-ui rounded-full transition-colors">
                    <X className="h-5 w-5" />
                  </button>
                </div>
                <div className="flex-1 overflow-y-auto p-4 custom-scrollbar">
                  {chatsLoading ? (
                    <div className="flex p-8 justify-center"><Loader2 className="animate-spin text-primary-ui" /></div>
                  ) : chats?.length === 0 ? (
                        ...
                        <div key={chat.id} className="group relative">
                          ...
                          >
                            <div className={cn(
                              ...
                              activeChatId === chat.id ? "bg-primary-ui/20" : "bg-secondary-ui"
                            )}>
                            ...
                            <div className="flex-1 overflow-hidden">
                              {editingChatId === chat.id ? (
                                <input
                                  ...
                                  onKeyDown={(e) => {
                                    if (e.key === "Enter") {
                                      renameChat.mutate({ chatId: chat.id, name: editName });
                                ...
                                <p className="text-[15px] font-bold truncate pr-6">{chat.name}</p>
                              )}
                              <p className="text-[11px] opacity-50 font-medium tracking-wider">{formatFernTime(chat.createdAt)}</p>
                            </div>
                          </button>
                                ...
                                <button
                                  onClick={(e) => {
                                    ...
                                    if (window.confirm("Are you sure you want to delete this session?")) {
                                      ...
                                      if (activeChatId === chat.id) setActiveChatId("new");
                                    }
                                  }}
