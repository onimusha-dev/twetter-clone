  ...
  const sections = [
    {
      title: "Your Account",
      items: [
        { icon: User, label: "Account Information", desc: "See your account details, email, and phone number", href: "/settings/account" },
        { icon: Lock, label: "Change your password", desc: "Change your password at any time", href: "/settings/password" },
      ]
    },
    {
       title: "Zerra Blue",
       items: [
         { icon: Palette, label: "Premium Preferences", desc: "Manage your premium membership and theme settings", href: "/settings/premium" },
       ]
    },
    {
      title: "Navigation",
      items: [
        { icon: Bell, label: "Notifications", desc: "Select the kinds of notifications you get about your activities", href: "/settings/notifications" },
        { icon: Eye, label: "Privacy and safety", desc: "Manage what information you allow other people on Zerra to see", href: "/settings/privacy" },
      ]
    }
  ];
      ...
      <div className="flex flex-col pb-20">
        <div className="p-6 border-b bg-secondary-ui/5">
          ...
          <p className="text-secondary-foreground opacity-60 text-xs mb-6">
            Choose your perfect view. These settings affect all Zerra accounts on this browser.
          </p>
          <div className="flex flex-wrap gap-4">
            {THEMES.map((t) => (
              ...
              >
                <div className={cn("h-10 w-10 rounded-full border-2 shadow-lg", t.class)} />
                <span className="text-[10px] font-medium uppercase tracking-wider">{t.name}</span>
              </button>
            ))}
          </div>
        </div>
        ...
        {sections.map((section, idx) => (
          <div key={section.title} className={cn("flex flex-col", idx > 0 && "mt-2")}>
            <h3 className="px-4 py-3 text-lg font-bold text-foreground">{section.title}</h3>
            {section.items.map((item) => (
              ...
              >
                ...
                <div className="flex flex-col items-start text-left">
                  ...
                  <span className="text-xs text-secondary-foreground opacity-60">{item.desc}</span>
                </div>
        ...
        <div className="mt-8 px-4">
           ...
           >
             ...
             <div className="flex flex-col items-start text-left">
                ...
                <span className="text-xs opacity-70 italic">End your current session on this device</span>
             </div>
           </button>
        </div>
      </div>
