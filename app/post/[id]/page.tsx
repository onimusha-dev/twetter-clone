      ...
      <div className="flex h-14 items-center border-b px-4 sticky top-0 bg-background/80 backdrop-blur-md z-20 gap-4">
        ...
        <div className="flex flex-col min-w-0">
          <div className="flex items-center gap-1">
            <h2 className="text-xl font-bold leading-tight truncate">{(post as any)?.author?.name || (post as any)?.user?.name || "Post"}</h2>
            {((post as any)?.author?.isVerified || (post as any)?.user?.isVerified) && <VerificationBadge size={18} />}
          </div>
      ...
      <div className="flex flex-col min-h-[calc(100vh-3.5rem)]">
        {postLoading ? (
          ...
          <>
            ...
            {/* Reply Input Box */}
            <div id="comments" className="w-full border-b p-4 flex flex-col gap-3 scroll-mt-20">
              <div className="flex gap-3 w-full">
                ...
                <div className="flex grow flex-col gap-2">
                  ...
                  <div className="flex w-full items-center justify-end mt-2 pt-2 border-t">
                    ...
                    >
                      {isCommenting ? <Loader2 className="h-4 w-4 animate-spin outline-none" /> : "Reply"}
                    </button>
                  </div>
                </div>
              </div>
