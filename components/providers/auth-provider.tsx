  ...
  useEffect(() => {
    const verifyAuth = async () => {
      ...
      try {
        ...
        console.log("[AuthProvider] Verification result:", { status: response.data?.status, hasUser: !!userData });
        
        if (userData && (userData.username || userData.id)) {
          setUser(userData);
        }
      } catch (error: any) {
        console.error("[AuthProvider] Verification error:", error.response?.status || error.message);
