...
import { Home, Search, Bell, Mail, Bookmark, User, Settings, MoreHorizontal, Sparkles, Plus } from "lucide-react";
          ...
          <nav className="flex w-full items-center justify-around sm:mt-4 sm:flex-col sm:items-end md:items-start">
            {navItems.map((item) => (
              ...
              >
                ...
                <span className="hidden text-xl font-medium md:block">{item.label}</span>
              </Link>
            ))}
          </nav>
            ...
            {isAuthenticated && user ? (
              <div className="flex w-full items-center gap-3 rounded-full p-3 transition-colors hover:bg-secondary-ui md:px-3 cursor-pointer group relative">
                <div className="h-10 w-10 shrink-0 rounded-full bg-accent-ui flex items-center justify-center overflow-hidden border border-border-ui">
                  {user.avatar ? <img src={getMediaUrl(user.avatar)} alt={user.name} className="h-full w-full object-cover" /> : <User className="h-6 w-6 opacity-40" />}
                </div>
                <div className="hidden flex-col md:flex">
                  ...
                  <span className="text-xs text-secondary-foreground opacity-60 truncate max-w-[120px]">@{user.username}</span>
                </div>
                ...
                <div className="flex flex-col gap-4">
                  {[1, 2, 3].map((i) => (
                    <div key={i} className="flex flex-col">
                      <span className="text-xs text-secondary-foreground opacity-60">Trending in Tech</span>
                      <span className="font-bold">#ZerraPremium</span>
                      <span className="text-xs text-secondary-foreground opacity-60">12.4K Posts</span>
                    </div>
                  ))}
                </div>
              </div>
