...
export default function PostCard({ post, className, isDetailedView = false }: PostCardProps) {
  const { author: postAuthor, user: postUser, content, media, createdAt, _count, id } = post as any;
  ...
  const handleUpdate = () => {
    ...
    updatePost({ postId: id, content: editContent, published: isPublished }, {
    ...
    >
      <Link href={author ? `/profile/${author.username}` : "#"} className="shrink-0">
        <div className="h-10 w-10 overflow-hidden rounded-full bg-accent-ui border border-border-ui">
          {avatarUrl ? (
            <img src={getMediaUrl(avatarUrl)} alt={author?.name || "Logo"} className="h-full w-full object-cover select-none" />
          ) : (
      ...
      <div className="flex flex-1 flex-col gap-1 min-w-0">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-1.5 text-[15px] min-w-0">
            ...
            {author ? (
              <>
                <Link href={`/profile/${author.username}`} className="font-bold hover:underline flex items-center gap-1 truncate">
                  <span className="truncate">{author.name}</span>
                  {author.isVerified && <VerificationBadge size={14} />}
                </Link>
                <span className="text-secondary-foreground opacity-50 truncate">@{author.username}</span>

              </>
            ) : (
              <span className="text-secondary-foreground opacity-50">Unknown User</span>
            )}
              ...
              <div className="flex justify-between items-center px-1">
                <div className="flex items-center gap-4">
                  <span className={cn(
                    ...
                    !user?.isVerified && editContent.length > 500 ? "text-red-500" : "text-secondary-foreground opacity-40"
                  )}>
                    {editContent.length}{!user?.isVerified && " / 500"}
                  </span>
                  <label className="flex items-center gap-1.5 cursor-pointer group/toggle">
                    ...
                    <div className={cn(
                      ...
                      isPublished ? "bg-primary-ui" : "bg-secondary-ui border border-border-ui"
                    )}>
                      <div className={cn(
                        "absolute top-0.5 left-0.5 w-3 h-3 rounded-full transition-transform",
                        isPublished ? "translate-x-3 bg-background" : "bg-foreground/40"
                      )} />
                    </div>
                ...
                <div className="flex gap-2">
                  ...
                  <button 
                    ...
                    disabled={isUpdating || !editContent.trim() || (editContent === content && isPublished === (post.published ?? true)) || (!user?.isVerified && editContent.length > 500)}
          ...
          >
            ...
            {media && (
              <div className="mt-3 overflow-hidden rounded-2xl border border-border-ui">
                <img src={getMediaUrl(media)} alt="Post content" className="max-h-[500px] w-full object-cover" />
              </div>
            )}
          </div>
        )}
        ...
        <div className="mt-3 flex max-w-md justify-between text-secondary-foreground opacity-60">
          <Link href={`/post/${id}#comments`} className="group flex items-center gap-2 transition-colors hover:text-primary-ui">
