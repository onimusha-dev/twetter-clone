...
export default function ProfileHeader({ profile, isOwn }: ProfileHeaderProps) {
  ...
  return (
    ...
    <div className="flex flex-col border-b">
      {/* Top Navigation */}
      <div className="flex h-14 items-center gap-6 px-4 sticky top-0 bg-background/80 backdrop-blur-md z-20 select-none">
        <button 
          onClick={() => router.back()}
      ...
      <div className="relative h-48 w-full bg-accent-ui/20 overflow-hidden">
        {profile.banner ? (
          <img src={getMediaUrl(profile.banner)} alt="Banner" className="h-full w-full object-cover select-none" />
        ) : (
      ...
      <div className="relative px-4 pb-4">
        <div className="flex justify-between items-start">
          <div className="relative -mt-16 sm:-mt-20">
            <div className="h-32 w-32 sm:h-36 sm:w-36 rounded-full border-4 border-background bg-secondary-ui overflow-hidden flex items-center justify-center">
              {profile.avatar ? (
                <img src={getMediaUrl(profile.avatar)} alt={profile.name} className="h-full w-full object-cover bg-background select-none" />
              ) : (
                <span className="text-4xl font-bold opacity-20 uppercase">{profile.username[0]}</span>
              )}
            </div>
          </div>
          <div className="mt-3">
            {isOwn ? (
              <button 
                ...
                className="rounded-full border px-4 py-1.5 font-bold hover:bg-secondary-ui transition-colors"
              >
                Edit profile
              </button>
            ) : (
              <button className={cn(
                "rounded-full px-5 py-1.5 font-bold transition-colors",
                profile.isFollowing 
        ...
        <div className="mt-4 flex flex-col gap-3">
            <div className="flex flex-col">
              ...
              <span className="text-secondary-foreground opacity-60">@{profile.username}</span>
            </div>
          ...
          <div className="flex flex-wrap gap-4 text-secondary-foreground opacity-70 text-sm">
            {profile.link && (
              <div className="flex items-center gap-1">
                ...
                <a href={profile.link} target="_blank" rel="noreferrer" className="text-primary-ui hover:underline truncate max-w-[200px]">
            ...
            <div className="flex items-center gap-1">
              ...
              <span>{profile.createdAt ? `Joined ${new Date(profile.createdAt).toLocaleDateString()}` : "Joined recently"}</span>
            </div>
          </div>
          ...
          <div className="flex gap-5 text-sm">
            <button className="hover:underline flex gap-1 items-center">
              ...
              <span className="text-secondary-foreground opacity-60">Following</span>
            </button>
            <button className="hover:underline flex gap-1 items-center">
              <span className="font-bold text-foreground">{profile.followersCount}</span>
              <span className="text-secondary-foreground opacity-60">Followers</span>
            </button>
          </div>
        </div>
      </div>
