...
export const useProfile = (username: string) => {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      console.log(`[useProfile] RAW Response for ${username}:`, JSON.stringify(response.data, null, 2));
...
export const useUpdateProfile = () => {
  ...
  return useMutation({
    mutationFn: async (data: {
      ...
      const response = await api.patch<{ status: string; message: string; data: ProfileResponse["data"] }>(
        "/users/me",
        formData,
