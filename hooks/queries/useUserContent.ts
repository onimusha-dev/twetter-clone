  ...
  return useInfiniteQuery({
    ...
    queryFn: async ({ pageParam }) => {
      ...
      const [postsRes, articlesRes] = await Promise.all([
        api.get(`/posts/author/${userId}`, { params: { limit, cursor: pageParam } }).catch(() => ({ data: { success: false, data: [] } })),
        api.get(`/articles/user/${userId}`, { params: { limit, cursor: pageParam } }).catch(() => ({ data: { success: false, data: [] } }))
      ]);

      const items: UserFeedItem[] = [];

      if ((postsRes.data?.success || postsRes.data?.status === "success") && Array.isArray(postsRes.data.data)) {
        postsRes.data.data.forEach((post: Post) => {
          items.push({ type: "post", data: post });
        });
      }

      if ((articlesRes.data?.success || articlesRes.data?.status === "success") && Array.isArray(articlesRes.data.data)) {
