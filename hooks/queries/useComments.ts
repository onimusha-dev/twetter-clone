...
export const useCreateComment = () => {
  ...
  return useMutation({
    mutationFn: async ({ content, postId, articleId }: { content: string, postId?: number, articleId?: number }) => {
    ...
    onSuccess: async (_, variables) => {
      const promises = [];
      if (variables.postId) {
        promises.push(queryClient.invalidateQueries({ queryKey: ["comments", "post", variables.postId] }));
        promises.push(queryClient.invalidateQueries({ queryKey: ["post", variables.postId] })); // Update post reply count
      }
      if (variables.articleId) {
        promises.push(queryClient.invalidateQueries({ queryKey: ["comments", "article", variables.articleId] }));
      }
