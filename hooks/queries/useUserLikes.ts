...
export const useUserLikes = (username: string | undefined, userId?: number, options: QueryOptions = {}) => {
  const limit = 10;
        ...
        try {
          ...
          if (data.status === "success" || data.success) {
            ...
            return rawItems.map((item: any) => {
              ...
              if (item.title || item.body) return { type: "article", data: item } as LikedItem;
              return null;
